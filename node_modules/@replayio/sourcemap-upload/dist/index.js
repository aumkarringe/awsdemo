...
const debug = (0, debug_1.default)("replay:sourcemap-upload");
async function uploadSourceMaps(opts) {
    var _a, _b, _c, _d;
    (0, assert_1.default)(Array.isArray(opts.filepaths)
        ? opts.filepaths.every((p) => typeof p === "string")
        : typeof opts.filepaths === "string", "'filepaths' must be a string or array of strings");
    (0, assert_1.default)(typeof opts.group === "string", "'group' must be a string");
    (0, assert_1.default)(typeof opts.key === "string" || opts.key === undefined, "'key' must be a string or undefined");
    (0, assert_1.default)(typeof opts.dryRun === "boolean" || opts.dryRun === undefined, "'dryRun' must be a boolean or undefined");
    (0, assert_1.default)(typeof opts.deleteAfterUpload === "boolean" ||
        opts.deleteAfterUpload === undefined, "'deleteAfterUpload' must be a boolean or undefined");
    (0, assert_1.default)(typeof opts.matchSourcemapsByFilename === "boolean" ||
        opts.matchSourcemapsByFilename === undefined, "'matchSourcemapsByFilename' must be a boolean or undefined");
    (0, assert_1.default)((Array.isArray(opts.extensions) &&
        opts.extensions.every((ext) => typeof ext === "string")) ||
        opts.extensions === undefined, "'extensions' must be an array of strings or undefined");
    (0, assert_1.default)(!opts.extensions || opts.extensions.length > 0, "'extensions' must not be empty");
    (0, assert_1.default)((_b = (_a = opts.extensions) === null || _a === void 0 ? void 0 : _a.every((ext) => !glob_1.default.hasMagic(ext))) !== null && _b !== void 0 ? _b : true, "'extensions' entries may not contain special glob chars");
    (0, assert_1.default)((_d = (_c = opts.extensions) === null || _c === void 0 ? void 0 : _c.every((ext) => ext.startsWith("."))) !== null && _d !== void 0 ? _d : true, "'extensions' entries must start with '.'");
    (0, assert_1.default)((Array.isArray(opts.ignore) &&
        opts.ignore.every((pattern) => typeof pattern === "string")) ||
        opts.ignore === undefined, "'ignore' must be an array of strings or undefined");
    (0, assert_1.default)(typeof opts.root === "string" || opts.root === undefined, "'root' must be a string or undefined");
    const apiKey = opts.key ||
        process.env.REPLAY_API_KEY ||
        process.env.RECORD_REPLAY_API_KEY ||
        null;
    (0, assert_1.default)(apiKey, "'key' must contain a key, or the REPLAY_API_KEY must be set.");
    (0, assert_1.default)(typeof opts.log === "function" || opts.log === undefined, "'log' must be a function or undefined");
    const apiServer = opts.server ||
        process.env.RECORD_REPLAY_API_SERVER ||
        "https://api.replay.io";
    (0, assert_1.default)(typeof apiServer === "string" || "'apiServer' must be a string");
...
async function processSourceMaps(opts) {
    ...
    await (0, p_map_1.default)(mapsToUpload, async (mapToUpload) => {
        const { relativePath, absPath } = mapToUpload;
        debug("Uploading %s", absPath);
        log("normal", `Uploading ${relativePath}`);
        if (!dryRun) {
            await uploadSourcemapToAPI(groupName, apiKey, mapToUpload, opts.apiServer, opts.userAgentAddition);
        }
        if (deleteAfterUpload) {
            log("normal", `Deleting ${relativePath}`);
            if (!dryRun) {
                await fs_1.default.promises.unlink(absPath);
            }
        }
...
async function sendUploadPUT(opts) {
    return (0, node_fetch_1.default)(`${opts.apiServer}/v1/sourcemap-upload`, {
        method: "PUT",
        headers: {
...
async function uploadSourcemapToAPI(groupName, apiKey, map, apiServer, userAgentAddition) {
    let response;
    try {
...
async function findAndResolveMaps(opts) {
    var _a;
    const { cwd, filepaths, extensions, ignorePatterns, matchSourcemapsByFilename = true, } = opts;
    ...
    for (const fileArg of filepaths) {
        ...
        for (const absPath of await listAllFiles(absFileArg, ignorePatterns, extensions)) {
            ...
            const fileURL = (0, url_1.pathToFileURL)(absPath).toString();
            if (map !== undefined) {
                ...
                const matches = (0, string_prototype_matchall_1.default)(trailingComments, /\/\*(?:[@#] *sourceMappingURL=(.*)\s*|[\s\S]*?)\*\/|\/\/(?:[@#] *sourceMappingURL=(.*)|.*?)(?:\r?\n|$)|\r?\n/g);
                debug("hashed filepath: %s", absPath);
                const possibleMatches = Array.from(matches, (match) => { var _a; return (_a = (match[1] || match[2])) === null || _a === void 0 ? void 0 : _a.trim(); });
                ...
                if (matchSourcemapsByFilename) {
                    ...
                    if (fs_1.default.existsSync(mapAbsolutePath)) {
                        const mapURL = (0, url_1.pathToFileURL)(mapAbsolutePath).toString();
...
async function listAllFiles(absPath, ignorePatterns, extensions) {
    ...
    if (stat.isFile()) {
        ...
        (0, assert_1.default)(extensions.every((ext) => !glob_1.default.hasMagic(ext)));
...
function getUserAgent(userAgentAddition) {
