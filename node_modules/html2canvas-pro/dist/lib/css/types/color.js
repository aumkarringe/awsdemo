...
exports.color = {
    ...
    parse: function (context, value) {
        ...
        if (value.type === 5 /* HASH_TOKEN */) {
            ...
            return color_utilities_1.pack(r, g, b, a);
        }
...
var rgb = function (_context, args) {
    ...
    if (color_utilities_1.isRelativeTransform(tokens)) {
    ...
    if (tokens.length === 3) {
        ...
        return color_utilities_1.pack(r, g, b, 1);
    }
    if (tokens.length === 4) {
...
var _color = function (context, args) {
    var tokens = args.filter(parser_1.nonFunctionArgSeparator), token_1_value = tokens[0].type === 20 /* IDENT_TOKEN */ ? tokens[0].value : 'unknown', is_absolute = !color_utilities_1.isRelativeTransform(tokens);
    if (is_absolute) {
        ...
        var c1 = parser_1.isNumberToken(tokens[1]) ? tokens[1].number : 0, c2 = parser_1.isNumberToken(tokens[2]) ? tokens[2].number : 0, c3 = parser_1.isNumberToken(tokens[3]) ? tokens[3].number : 0, a = tokens.length > 4 &&
            tokens[4].type === 6 /* DELIM_TOKEN */ &&
            tokens[4].value === '/' &&
            parser_1.isNumberToken(tokens[5])
            ? tokens[5].number
            : 1;
                ...
                for (var _i = 0, parts_1 = parts; _i < parts_1.length; _i++) {
                    ...
                    expression +=
                        part.type === 18 /* FUNCTION */ && part.name === 'calc'
                            ...
                            : parser_1.isNumberToken(part)
                                ? part.number
                                : part.type === 6 /* DELIM_TOKEN */ || parser_1.isIdentToken(part)
                                    ? part.value
                                    : '';
                }
                expression += ')';
        ...
        var from_colorspace = tokens[1].type === 18 /* FUNCTION */
            ? tokens[1].name
            : parser_1.isIdentToken(tokens[1]) || tokens[1].type === 5 /* HASH_TOKEN */
                ? 'rgb'
                : 'unknown', to_colorspace = parser_1.isIdentToken(tokens[2]) ? tokens[2].value : 'unknown';
        var from = tokens[1].type === 18 /* FUNCTION */ ? tokens[1].values : parser_1.isIdentToken(tokens[1]) ? [tokens[1]] : [];
        if (parser_1.isIdentToken(tokens[1])) {
            var named_color = exports.COLORS[tokens[1].value.toUpperCase()];
            if (typeof named_color === 'undefined') {
                throw new Error("Attempting to use unknown color in relative color 'from'");
            }
            else {
                var _c = exports.parseColor(context, tokens[1].value), alpha = 0xff & _c, blue = 0xff & (_c >> 8), green = 0xff & (_c >> 16), red = 0xff & (_c >> 24);
                from = [
        ...
        var from_color = fromColorToXyz(context, from), from_final_colorspace = toColorFromXyz(from_color), c1 = extractComponent(from_final_colorspace, tokens[3]), c2 = extractComponent(from_final_colorspace, tokens[4]), c3 = extractComponent(from_final_colorspace, tokens[5]), a = tokens.length > 6 &&
            ...
            tokens[6].value === '/' &&
            parser_1.isNumberToken(tokens[7])
            ? tokens[7].number
            : 1;
